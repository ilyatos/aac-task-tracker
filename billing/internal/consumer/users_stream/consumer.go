...
func New(createUserCommand *create_user_command.Command, updateUserCommand *update_user_command.Command) *Consumer {
...
func (c *Consumer) handleUserCreated(ctx context.Context, event []byte) error {
	...
	case *user_created.UserCreated_V1:
		...
		return c.createUserCommand.Handle(ctx, create_user_command.CreateUserData{
			...
			Name:     v1.GetName(),
			Email:    v1.GetName(),
			Role:     user_model.Role(strings.ToLower(user_event.Role_name[int32(v1.GetRole())])),
		})
	}
